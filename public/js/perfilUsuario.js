...
if (btnAbrir) {
    btnAbrir.addEventListener('click', function () {
        ...
        if (menuSide.classList.contains('recolhido')) {
            logo.src = "/imgs/perfilUsuario/logoFocinho.png";
        } else {
            logo.src = "/imgs/perfilUsuario/logoFarejo.png";
        }
    });
}
...
const botoes = document.querySelectorAll(".btn");
    ...
    botoes.forEach(btn => {
      btn.addEventListener("click", () => {
        // se for do mesmo grupo, desmarca os outros
        const grupo = btn.parentElement;
        grupo.querySelectorAll(".btn").forEach(b => b.classList.remove("selected"));
    ...
    document.getElementById("salvar").addEventListener("click", () => {
      const selecionados = document.querySelectorAll(".btn.selected");
      let preferencias = [];

      selecionados.forEach(btn => {
        preferencias.push(btn.dataset.value);
      });

      document.getElementById("resultado").textContent = 
        "Preferências: " + (preferencias.length ? preferencias.join(", ") : "Nenhuma selecionada");
    });